import { OtherDomain, OtherDomainDocument } from '@app/schema';
import { Injectable, BadRequestException } from '@nestjs/common';
import { InjectModel } from '@nestjs/mongoose';
import { Model } from 'mongoose';
import { ICreateOtherDomainDTO, IUpdateOtherDomainDTO } from './dto/otherdomain';
...
export class OtherDomainService {
  ...
  async createOtherDomain(createOtherDomainDTO: ICreateOtherDomainDTO): Promise<void> {
  ...
  async get(params: any): Promise<any> {
    ...
    const result = await this.otherDomainModel.find(query).skip(skip).limit(limit).exec();
